...
class PendingActionsQueue extends EventEmitter {
  ...
  enqueue(type: "archive" | "trash", emailId: string, accountId: string): string {
    ...
    log.info(`Queued ${type} for email ${emailId} (${this.queue.length} pending)`);
    return id;
  }
  ...
  async processQueue(): Promise<void> {
    ...
    log.info(`Processing ${this.queue.length} pending action(s)`);
    ...
    for (const item of items) {
      if (!networkMonitor.isOnline) {
        ...
        log.info(`Went offline, ${this.queue.length} action(s) re-queued`);
        break;
      }
      ...
      try {
        ...
        if (isNetwork) {
          ...
          log.info(`Network error, ${this.queue.length} action(s) re-queued`);
          break;
        }
