  ...
  return (
    <div className="flex items-start gap-2 py-2">
      <div className="w-5 h-5 rounded-full bg-purple-100 dark:bg-purple-900/40 flex items-center justify-center flex-shrink-0 mt-0.5">
        <svg className="w-3 h-3 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
...
function shouldRenderAsCode(key: string, value: string): boolean {
  ...
  const sqlKeywords = /\b(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|FROM|WHERE|JOIN|GROUP BY|ORDER BY|HAVING|UNION|WITH)\b/i;
...
function ToolIcon({ name }: { name: string }) {
  ...
  if (lower.includes("query") || lower.includes("sql") || lower.includes("db") || lower.includes("database")) {
    // Database icon
    return (
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden my-2">
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">
        ...
        {!isDone && (
          <svg className="w-3.5 h-3.5 text-blue-500 animate-spin ml-auto" fill="none" viewBox="0 0 24 24">
      ...
      {inputEntries.length > 0 && (
        <div className="px-3 py-2 space-y-1.5">
          {inputEntries.map(([key, value]) => {
            ...
            const strValue = rawStr.length > INPUT_DISPLAY_LIMIT
            ...
            const displayValue = isTruncated && !inputExpanded
              ? strValue.slice(0, CODE_TRUNCATE_LIMIT)
              : strValue;
...
function ErrorBlock({ message, onAuthRequest, onRetry }: { message: string; onAuthRequest?: () => void; onRetry?: () => void }) {
...
function DoneBlock({ summary }: { summary: string }) {
  return (
  ...
  return (
    <div className="rounded-lg border border-purple-200 dark:border-purple-800/50 overflow-hidden my-2">
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300">
        ...
        {!isDone && (
          <svg className="w-3.5 h-3.5 text-purple-500 animate-spin ml-auto" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
        )}
...
function EventTimeline({ events, runFinished, onAuthRequest, onRetry }: EventTimelineProps) {
  ...
  const subAgentWrappers = new Map<string, { toolName: string; nestedEvents: ScopedAgentEvent[] }>();
    ...
    if (evt.type === "user_message") {
      ...
      renderedElements.push(
        <ErrorBlock
          ...
          message={evt.message}
          onAuthRequest={evt.message === "AGENT_AUTH_REQUIRED" && authProviderId && onAuthRequest ? () => onAuthRequest(authProviderId) : undefined}
...
export const AgentTabContent = memo(function AgentTabContent({ emailId }: { emailId: string }) {
  ...
  const handleAuth = useCallback(async (providerId: string) => {
    ...
    try {
      ...
      if (result?.success && result.data?.success && currentTask) {
        ...
        store.startAgentTask(newTaskId, emailId, currentTask.providerIds, currentTask.prompt, currentTask.context);
        await window.api?.agent?.run?.(newTaskId, currentTask.providerIds, currentTask.prompt, currentTask.context);
      }
    } finally {
  ...
  const handleRetry = useCallback(async () => {
    ...
    const result = await window.api?.agent?.run?.(newTaskId, currentTask.providerIds, currentTask.prompt, currentTask.context) as
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      ...
      const result = await window.api?.drafts?.rerunAgent?.(emailId) as
        { success: boolean; data?: { taskId: string }; error?: string } | undefined;
  ...
  const handleFollowUp = useCallback(async () => {
    ...
    const result = await window.api?.agent?.run?.(newTaskId, currentTask.providerIds, compositePrompt, updatedContext) as
      { success: boolean; error?: string } | undefined;
    if (result && !result.success) {
      const store = useAppStore.getState();
      store.appendAgentEvent(newTaskId, {
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 dark:border-gray-700/50 flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {task.providerIds.length === 1 && (
            <span className="text-xs text-gray-400 dark:text-gray-500 truncate">
              {availableProviders.find((p) => p.id === task.providerIds[0])?.name ?? task.providerIds[0]}
            </span>
          )}
        </div>
        <div className="flex items-center gap-1">
          ...
          {task.status === "running" && (
            <button
              onClick={() => { window.api?.agent?.cancel?.(task.taskId); cancelAgentTask(task.taskId); }}
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-3">
        {Object.entries(task.runs).map(([providerId, run]) => {
          ...
          const runFinished = run.status === "completed" || run.status === "failed" || run.status === "cancelled";
        ...
        <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder={task.status === "running" ? "Send when ready..." : task.status === "failed" ? "Try again or follow up..." : "Follow up..."}
