...
const ICONS = {
  ...
  keyboard: "M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707",
    ...
    const allActions: CommandAction[] = [
      ...
      // --- Instant Intro ---
      {
        ...
        execute: () => {
          ...
          const introducedEmails = allRecipients
            .map(parseEmail)
            .filter((e) => {
          ...
          const introducedNames = uniqueIntroduced.map((addr) => {
            const original = allRecipients.find((r) => parseEmail(r).toLowerCase() === addr.toLowerCase());
          ...
          const greeting = introducedFirstNames.length > 2
        ...
        execute: () => {
          ...
          window.api.sync.getEmails(account.id).then((result: IpcResponse<DashboardEmail[]>) => {
            if (result.success && result.data) {
              const otherEmails = useAppStore.getState().emails.filter(
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-gray-800 rounded-xl shadow-2xl dark:shadow-black/40 overflow-hidden border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
          ...
          />
          <kbd className="px-2 py-0.5 text-xs text-gray-400 bg-gray-100 dark:bg-gray-700 rounded">esc</kbd>
        </div>
        ...
        <div className="flex items-center gap-4 px-4 py-2 text-xs text-gray-400 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          <span>
            <kbd className="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 rounded">&uarr;&darr;</kbd> navigate
          </span>
          <span>
